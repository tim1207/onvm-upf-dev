...
Status UpfContextInit() {
    ...
    //XXX:vivek ListInit(&self.gtpv1DevList);
    //XXX:vivek ListInit(&self.gtpv1v6DevList);
    ...
    //XXX:vivek PfcpNodeInit(); // init pfcp node for upfN4List (it will used pfcp node)
    //XXX:vivek TimerListInit(&self.timerServiceList);
...
Status UpfContextTerminate() {
    ...
    //XXX:vivek  PfcpRemoveAllNodes(&self.upfN4List);
    //XXX:vivek PfcpNodeTerminate();

    // TODO: remove gtpv1TunnelList, ranS1uList, upfN4LIst, apnList,
    // pdrList, farList, qerList, urrLIist
    //XXX:vivek Gtpv1DevListFree(&self.gtpv1DevList);
    //XXX:vivek Gtpv1DevListFree(&self.gtpv1v6DevList);
    //XXX:vivek SockNodeListFree(&self.pfcpIPList);
    //XXX:vivek SockNodeListFree(&self.pfcpIPv6List);
...
Status UpfSessionRemove(UpfSession *session) {
    ...
    while (pdrIdPtr) {
        //XXX:vivek Gtpv1TunDevNode *gtpv1Dev4 =
        //XXX:vivek   (Gtpv1TunDevNode*)ListFirst(&Self()->gtpv1DevList);
        //XXX:vivek UTLT_Assert(gtpv1Dev4, return STATUS_ERROR, "No GTP Device");
        //XXX:vivek Status status = GtpTunnelDelPdr(gtpv1Dev4->ifname, pdrIdPtr->pdrId);
        //XXX:vivek UTLT_Assert(status == STATUS_OK, ,
        //XXX:vivek             "Remove PDR[%u] failed", pdrIdPtr->pdrId);
